...
The <code>read.traps</code> function takes a data frame with x, y coordinates for each detector and creates
 ...
 then the traps are called "multi" type. <code>openpopscr</code> does
...
The data object requires a mesh. This data does not have a custom mesh, so I create one using <code>make.mask</code>: 
```{r make_mask}
...
One thing to note is that the <code>area</code> function returns the area in square kilometres, not hectares. Hectares are the standard unit of area for the <code>secr</code> package. 
...
Notice that m=1 is included in the function. If this is not included, the function returns the x and y coordinates for every mesh-point. 