...
<code>RcppParallel::setThreadOptions(numThreads = 7)</code>. The documentation and examples included assume that the user is using a machine with 8 available cores and that one core is not used to allow further use of the computer. To find out the number of cores your machine has available run the following code: <code>RcppParallel::defaultNumThreads()</code>. For further information about RcppParallel, [see the package's CRAN webpage](https://cran.r-project.org/web/packages/RcppParallel/index.html). 
